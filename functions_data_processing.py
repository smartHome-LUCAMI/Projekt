...
=======
    ...
    for interval in hour_span:
        mask = (data_frame.index.hour >= interval[0]) & (data_frame.index.hour <= interval[1])
        data_frame[attribute].mask(cond = mask, other = 0.9, inplace =True)
        
    star_index_date = data_frame.index.get_loc(graphing_interval[0])
    end_index_date = data_frame.index.get_loc(graphing_interval[1])

    sns.lineplot(x = data_frame.index[star_index_date: end_index_date], y = attribute, data = data_frame[star_index_date:end_index_date])
...
=======
    ...
    sns.lineplot(x = data_frame.index[star_index_date: end_index_date], y = 'Ground Truth', data = data_frame[star_index_date:end_index_date])
...
=======
    ...
    data_frame[attribute].mask(cond = (data_frame[attribute] >= treshold), inplace=True, other= 0.9)
    data_frame[attribute].mask(cond = (data_frame[attribute] <= treshold), inplace=True, other= 0.1)
...
def pre_process(data_frame, attribute_1, attribute_2, attribute_3, graphing_interval):
    ...
    sns.lineplot(x = data_frame.index[star_index_date: end_index_date], y =  attribute_3, data = data_frame[star_index_date:end_index_date])
...
=======
    ...
    for i in pd.date_range(time_span[0], time_span[1], freq='19min'):
        for j in pd.date_range(start = i, end = i + pd.Timedelta(treshold_interval, 'm'), freq="min"):
    
            if((data_frame.loc[i, attribute]==0.9) & (data_frame.loc[j, attribute]==0.9)):
               data_frame.loc[i:j, attribute] = 0.9
